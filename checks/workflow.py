...
def upgrade(api):
    ...
    require(detail["applicability"] == {"applicable": True, "reasons": []}, "fresh ready plan should be applicable")
    release(api, "atlas-core", "3.0.0")
    detail = api.request("GET", path)
    require(detail["applicability"]["applicable"] is False, "stale plan still applicable")
    require(detail["applicability"]["reasons"] == ["catalog_changed"], "catalog change not reported")
    require(detail["state"] == "ready" and detail["revision"] == ready["revision"], "applicability read mutated the plan")
    blocked = api.request("GET", "/api/v1/plans?applicability=not_applicable")
    require(blocked["total"] == 2 and all(not item["applicability"]["applicable"] for item in blocked["items"]), "not_applicable filter is wrong")
    require(api.request("GET", "/api/v1/plans?applicability=applicable")["total"] == 0, "stale plan still counts as applicable")
    require(api.request("GET", "/api/v1/plans?state=ready&applicability=not_applicable")["total"] == 1, "combined state and applicability filter is wrong")
    paged = api.request("GET", "/api/v1/plans?applicability=not_applicable&limit=1")
    require(paged["total"] == 2 and len(paged["items"]) == 1, "filtered pagination is wrong")
    api.request("GET", "/api/v1/plans?applicability=sometimes", expected=400)
    ...
    require(api.request("GET", path)["applicability"]["applicable"] is True, "revalidated plan should be applicable")
    ...
    detail = api.request("GET", path)
    require(detail["applicability"]["reasons"] == ["state_not_ready", "environment_changed"], "applied plan reasons are wrong")
    other = "/api/v1/plans/" + second["id"]
    api.request("POST", other + "/validate", {"revision": 1}, 409)
    detail = api.request("GET", other)
    require(detail["applicability"]["reasons"] == ["state_not_ready", "environment_changed"], "stale draft reasons are wrong")
    cancelled = api.request("POST", other + "/cancel", {"revision": 1})
    require(cancelled["state"] == "cancelled", "plan cancellation failed")
    follow = api.request("POST", "/api/v1/plans", {"environment_id": "integration", "base_revision": 2, "roots": {"render-engine": "2.0.0"}, "reason": "跟踪环境变化"}, 201)
    bump = api.request("POST", "/api/v1/plans", {"environment_id": "integration", "base_revision": 2, "roots": {"render-engine": "2.0.0"}, "reason": "推进环境修订号"}, 201)
    api.request("POST", "/api/v1/plans/" + follow["id"] + "/validate", {"revision": 1})
    bump_ready = api.request("POST", "/api/v1/plans/" + bump["id"] + "/validate", {"revision": 1})
    api.request("POST", "/api/v1/plans/" + bump["id"] + "/apply", {"revision": bump_ready["revision"]})
    detail = api.request("GET", "/api/v1/plans/" + follow["id"])
    require(detail["applicability"]["reasons"] == ["environment_changed"], "environment change not reported on its own")
