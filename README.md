...
环境的每个修订都有来源记录，可回答“这个修订是怎么变成现在状态的”：

- `created`：环境创建时的初始求解，记录当时的目录修订、根依赖与解析集合。
- `plan_applied`：成功应用方案产生，记录方案标识、方案所依据的目录修订与环境基修订，以及当时的根依赖与解析集合；可凭 `plan_id` 继续查询方案详情。
- `baseline`：从旧格式数据迁移而来的环境只能从当前状态建立起点，更早的历史保持未知，不做补造。

来源记录与环境创建或方案应用在同一持久化提交内写入，一起成功或失败；应用被拒绝了结的方案不会留下来源记录。该能力只提供追溯查询，不执行回滚。
...
| GET /environments/{id}/provenance | 按修订倒序分页查询环境修订来源 |
| GET /environments/{id}/provenance/{revision} | 反查指定环境修订的来源记录 |
...
启动会校验 schema、引用关系、选择结果、修订来源与事件序号，损坏数据会使服务拒绝启动。可在服务停止后复制整个数据目录作备份，并在停止状态下恢复。状态格式当前为 schema 2；schema 1 的数据在启动时自动迁移，为既有环境按当前状态建立 `baseline` 来源起点。
...
这些是有界运行检查：启动临时 HTTP 服务、构造最小输入、验证公开 API 输出并清理数据。覆盖持久化重启、输入拒绝、版本撤回、回溯、兼容环、无解、方案验证、目录过期、环境过期、应用、取消、修订溯源及旧格式迁移。
