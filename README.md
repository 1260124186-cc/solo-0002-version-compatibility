...
| POST /components/{id}/rename | 重命名组件标识并同步改写全部引用 |
...
- 组件标识可通过 `POST /components/{id}/rename`（请求体 `{"new_id":"..."}`）改为尚未占用的合法标识。重命名在一次持久化提交内同步改写依赖约束、环境与方案的根依赖和解析集合、方案差异及历史事件，要么全部生效要么整体失败；目录修订号随之递增，旧的 ready 方案需重新验证后才能应用，环境与方案修订号及已应用方案的历史对应关系保持不变。改名事件以 `renamed` 动作记录，`detail` 字段保留原标识。
