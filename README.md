...
- 仅支持稳定 `major.minor.patch`，每段最大 4294967295，无前导零。支持 `*`、精确版本、`>=`、`<=`、`>`、`<`、`^`、`~` 及空格分隔的交集，并支持 `||` 分隔的并集（如 `^1.0.0 || ^2.0.0`，版本满足任一分支即匹配）；每个约束最多 8 个并集分支，每个分支最多 8 个条件。
- `^1.2.3` 表示至少 1.2.3 且小于 2.0.0；`^0.2.3` 小于 0.3.0；`^0.0.3` 小于 0.0.4；`~1.2.3` 小于 1.3.0。
- 不支持预发行标记、构建元数据或通配数字段。依赖组件必须已存在，允许先建立组件后逐个添加含环依赖的版本。
...
- 错误格式为 `{"error":{"code":"...","detail":"...","conflicts":[],"conflict":{}}}`，`conflicts` 与 `conflict` 仅无解时出现。`conflicts` 保持原有的字符串证据格式；`conflict` 是结构化冲突说明，`components` 列出卷入冲突的组件，`causes` 给出互相矛盾的条件及其来源（`source` 为 `root` 或 `parent`，父组件来源附带 `source_component` 与 `source_version`）。求解失败时收集冲突成因，再收敛为近似最小集合：删除其中任意一条条件后剩余约束即可同时满足，因此证据只保留真正互相矛盾的约束，而不是所有尝试过的分支；依赖环与多父组件共同约束同一组件的场景同样适用。两者最多各给出 8 条，并非完整不可满足证明。
