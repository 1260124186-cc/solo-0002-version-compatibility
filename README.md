...
`POST /api/v1/plans/preview` 在创建方案之前查看变化：请求体为 `environment_id`、`base_revision`（当前环境修订号）和完整期望 `roots`，响应给出求解后的 `resolved`、相对该环境的 `changes`，以及本次预览绑定的 `base_revision` 与 `catalog_revision`。预览只读取状态快照：不创建方案、不记录事件、不占用方案容量，多次预览互不影响。

预览与正式验证使用同一条求解与差异计算路径，输入也相同（环境标识、环境修订号、期望根依赖），因此在两个修订号都未变化的前提下，预览的 `changes` 与随后创建并验证方案得到的结果一致。预览结果只对响应中携带的两个修订号有效：目录或环境任一变化后必须重新预览；此后的创建、验证、应用请求若仍携带旧修订号会被 409 拒绝，从而强制回到预览。

与“直接创建后验证”的差异边界：

- 一致的部分：输入校验（根依赖格式、组件存在性、环境修订号匹配）、求解器与差异结果、错误码（400/404/409/422）完全相同。
- 预览不具备的部分：没有方案标识与状态机，不持久化求解结果，不产生事件，因此也不能被应用、取消或查询；它不检查方案容量，也不需要 `reason`。
- 验证额外具备的部分：把结果写入方案并转为 ready、记录事件、占用方案容量，并在提交时重新检查目录与环境修订号；只有验证后的方案才能应用。
...
| POST /plans/preview | 预览期望根依赖相对环境的变化，不写入状态 |
