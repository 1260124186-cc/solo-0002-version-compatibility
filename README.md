...
- 组件可见性分 `public`（默认）与 `internal`。internal 组件必须声明 `family`，只能被同族组件或其 `allowed_consumers` 白名单中的组件直接依赖；根请求不能直接命名 internal 组件，但可经同族公开入口传递解析。白名单引用的组件须先存在。
- 依赖边可写作约束字符串，或 `{"constraint":"^1.0.0","visibility":"internal"}`；边级 internal 声明不授予权限，只在目标确为 internal 且访问合法时成立。非法直接边在版本登记时返回 422；求解中非法候选触发回溯，无解时以 `visibility_denied`（422）返回证据。环依赖的每条边（含环回边）独立判断。
- 收紧组件策略不会使历史环境或已应用方案失效：启动只做结构校验，已存记录继续可读；新求解、方案验证与应用才强制当前策略。
...
- 400 表示输入错误；404 表示对象不存在；409 表示状态或修订号冲突；413 表示请求过大；415 表示媒体类型错误；422 表示无解、可见性被拒绝或预算耗尽；503 表示繁忙。内部持久化错误返回 500，不暴露磁盘路径。
...
启动会校验 schema、引用关系、选择结果与事件序号，损坏数据会使服务拒绝启动。可在服务停止后复制整个数据目录作备份，并在停止状态下恢复。状态格式当前为 schema 2：读取 schema 1 状态时做加法迁移（旧组件默认 public，环境与已应用方案无可见性修订号），首次写回时升级为 schema 2；旧记录在启动时只做结构校验，不被新可见性策略追溯否定。
...
python3 checks/workflow.py visibility
```

这些是有界运行检查：启动临时 HTTP 服务、构造最小输入、验证公开 API 输出并清理数据。覆盖持久化重启、输入拒绝、版本撤回、回溯、兼容环、无解、方案验证、目录过期、环境过期、应用及取消。`visibility` 额外覆盖同族公开入口、允许列表、根直接引用 internal、跨族环回边拒绝、边级 internal 声明、旧 schema 状态迁移启动，以及策略收紧后历史环境可读而新求解被拒绝。
