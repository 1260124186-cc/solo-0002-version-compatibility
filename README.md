...
版本和环境都带有通道属性：`stable`（默认）或 `preview`。通道在添加版本时声明并随版本持久化，环境在创建时声明通道偏好；两者之后都不可更改。通道只是求解时的选择偏好：版本号规则不变，仍只接受三段式稳定版本并拒绝预发行标记，预览通道不等于在版本号上加预发行后缀。

求解分两个阶段：先只使用偏好通道内的可用版本完整求解；只有通道内无解时，才回退到全部可用版本重试，且每个组件仍按"偏好通道优先、通道内版本降序"的确定顺序尝试。两个阶段共享求解预算，结果对相同输入始终一致。撤回是硬排除，先于通道偏好：已撤回版本在任何通道下都不会被选中，被环境使用的版本无论属于哪个通道都不能撤回。

求解响应的 `channel` 给出生效的偏好，`channel_fallback` 标记是否回退到了通道外，`channels` 给出每个入选版本所属的通道。版本列表随版本返回 `channel`，并可用 `?channel=stable|preview` 过滤；环境响应携带其通道偏好；方案验证使用环境声明的偏好，方案内入选版本的通道可从目录推导，不重复存储。
...
| GET、POST /components/{id}/releases | 按版本降序分页查询（可用 channel 过滤）、添加不可变版本 |
| POST /components/{id}/releases/{version}/withdraw | 使用空对象请求撤回未使用版本 |
| POST /resolve | 求解根依赖和传递依赖（可声明通道偏好） |
| GET、POST /environments | 分页查询、创建环境（可声明通道偏好）并求解初始集合 |
...
- 通道取值为 `stable` 或 `preview`，缺省 `stable`，版本与环境创建后通道不可更改。求解先在偏好通道内进行，通道内无解时整体回退到全部可用版本，回退中仍按偏好通道优先的确定顺序尝试；撤回版本先于通道规则被排除。
...
python3 checks/workflow.py channels
python3 checks/workflow.py upgrade
```

这些是有界运行检查：启动临时 HTTP 服务、构造最小输入、验证公开 API 输出并清理数据。覆盖持久化重启、输入拒绝、版本撤回、回溯、兼容环、无解、通道偏好与回退、方案验证、目录过期、环境过期、应用及取消。
