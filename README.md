...
根依赖始终表示完整期望集合，不是增量补丁。目录变化后，应重新验证 ready 方案。环境变化后，应使用新环境修订号建立新方案。重复应用、旧修订号及正在使用的版本撤回均返回 409。已取消方案是终态：任何验证、应用或再次取消请求都返回 409 冲突，与提交的修订号或原因无关；首次取消仍必须提供非空原因。取消原因随方案修订一同持久化，事件流中的 `cancelled` 与 `corrected` 事件均携带对应原因。
...
启动会校验 schema、引用关系、选择结果与事件序号，损坏数据会使服务拒绝启动。唯一的兼容迁移：旧版本留下、没有取消原因的已取消方案，在加载时补齐占位原因 `cancelled before cancellation reasons were recorded`，历史事件保持不变，随后照常校验；迁移结果随下一次成功写入落盘。可在服务停止后复制整个数据目录作备份，并在停止状态下恢复。状态格式当前为 schema 1，除上述补齐外不包含其他跨版本迁移机制。
