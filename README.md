...
携带 `{"continuation_token": "…", "step_budget": 50}` 再次调用即可续算；不提供 `step_budget` 表示在剩余全局上限内尽量一次算完。分页累计的**展开次数**仍受 `COMPAT_MAX_STEPS` 约束，到达总上限后的非终止工作返回与一次性求解相同的 422 `limit_exceeded`，不会因为分页而放大算力。

`step_budget` 计数的是“选择并展开一个组件”这类非终止工作；而“确认当前选择已满足全部需求”的终止观察不占用展开预算，但仍计入报告的 `steps`。因此给定预算 `b` 的一页会完成 `b` 次展开、在暂停响应中报告 `b+1` 步（下一次递归入口已计步、其展开留到下一页）。一条 N 个组件的依赖链一次性求解报告 N+1 步；用任意分页方式续算完成时同样报告 N+1 步，`resolved`、`edges` 与冲突证据逐项一致。若 N 次展开恰好等于 `COMPAT_MAX_STEPS`，最后的终止观察仍被放行（一次性和分页都在 N+1 步完成）；需要第 N+1 次展开才能继续时，一次性和分页都在同一累计点返回 422。
