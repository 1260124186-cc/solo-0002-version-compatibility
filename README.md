...
每次成功求解还附带 `proof`，逐项说明每个被选版本由哪些约束引入、满足了什么条件：

- `catalog_revision`：证明绑定的目录修订号。证明只在该修订号下成立；目录变化后读取环境或方案时，`proof_status` 变为 `stale`，旧证明仍保留为历史记录，但不得作为当前结论（应用旧方案返回 409 `proof_stale`，需重新验证）。
- `roots`：求解所用的完整根约束，复核时必须与请求一致。
- `selection`：按求解器实际决策顺序排列的选择链。每一步含 `component`、`version`、`order`，以及：
  - `introduced_by`：该组件进入求解范围时已生效、从而真正参与了版本选择的约束，来源是 `root`（根约束）或 `<组件>@<版本>`（声明该依赖的那个已选版本）；
  - `verified_against`：由更晚选定的版本声明的约束。它们不可能影响早先的决策，但所选版本仍然满足这些约束，因此单独列出以示完整；这也是环边的落点。
- `cycles`：已选依赖图中的环的闭合表示。每个环以字典序最小节点为起点列出 `nodes`（每个组件只出现一次），`edges` 包含指回起点的闭合边，因此环是有界记录而不会无限展开。环的枚举有数量和步数上限，构建证明与复核证明使用同一个确定函数，截断结果可严格比对。

`proof_status` 不持久化，每次读取时按当前目录修订重算：`current`（证明绑定的修订仍在生效）、`stale`（目录已变化，证明仅作历史）、`absent`（draft 方案或尚无解析结果）。
...
- 错误格式为 `{"error":{"code":"...","detail":"...","conflicts":[]}}`。422 的 `no_solution` 携带最多 8 条搜索中遇到的约束证据（`conflicts`），它是失败路径上收集的人为可读线索，既不完整也不可独立复核；证明则不同，仅在成功时产生，是一条完整、机器可重放的选择链，每一步都能从目录重新推导并逐条验证约束匹配。
- `proof`（选择链）与 `edges`（依赖边）也不同：`edges` 只描述最终选定版本之间存在哪些带约束的边，不说明版本为何被选、约束在决策时是否已生效；证明补充了决策顺序、每条边/根约束归属的引入版本以及环的闭合位置。
- 400 表示输入错误；404 表示对象不存在；409 表示状态或修订号冲突（含 `proof_stale`：证明绑定的目录修订已失效）；413 表示请求过大；415 表示媒体类型错误；422 表示无解或预算耗尽；503 表示繁忙。内部持久化错误返回 500，不暴露磁盘路径。
...
启动会校验 schema、引用关系、选择结果、解析证明与事件序号，损坏数据会使服务拒绝启动。环境以及 ready/applied 方案的证明会做结构性复核（允许绑定旧目录修订，但选择链必须仍能从目录重新推导）；应用方案时则做严格复核：证明必须绑定当前目录修订且所有版本仍可用。可在服务停止后复制整个数据目录作备份，并在停止状态下恢复。状态格式当前为 schema 1，不包含跨版本迁移机制。
...
- `internal/resolution`：回溯求解、约束证据、解析证明（选择链与环闭合）及版本差异。
