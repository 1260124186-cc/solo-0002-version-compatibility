...
启动会校验 schema、引用关系、选择结果与事件序号，损坏数据会使服务拒绝启动。早期版本写入的环境缺少 `name_revision` 字段，启动时按初始名称修订号 1 自动补齐并照常校验，其余数据不受影响；除该字段外不提供跨版本迁移机制。可在服务停止后复制整个数据目录作备份，并在停止状态下恢复。状态格式当前为 schema 1。
...
python3 checks/workflow.py compat
```

这些是有界运行检查：启动临时 HTTP 服务、构造最小输入、验证公开 API 输出并清理数据。覆盖持久化重启、输入拒绝、版本撤回、回溯、兼容环、无解、方案验证、目录过期、环境过期、应用及取消。compat 额外需要 git 与 Go 工具链：它从仓库根提交构建基线二进制生成不含名称修订号的旧数据，再用当前服务直接启动，验证旧环境按初始名称修订号读取、旧环境修订的方案仍可应用、事件保留且可继续改名。
