...
func (s *Service) CreatePlan(ctx context.Context, input domain.PlanInput) (domain.Plan, error) {
	...
	at := now()
	plan := domain.Plan{ID: id, EnvironmentID: input.EnvironmentID, BaseRevision: input.BaseRevision, Revision: 1, Roots: domain.CopyStrings(input.Roots), Resolved: make(map[string]string), Changes: make([]domain.Change, 0), RootChanges: make([]domain.Change, 0), State: domain.Draft, Reason: input.Reason, CreatedAt: at, UpdatedAt: at}
...
func (s *Service) ValidatePlan(ctx context.Context, id string, revision uint64) (domain.Plan, error) {
	...
	rootChanges := domain.RootDiff(env.Roots, plan.Roots)
